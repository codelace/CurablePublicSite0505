...
const ValueCard: React.FC<ValueCardProps> = ({ value, isHovered }) => {
  const iconMap = {
    transparency: (
      <svg className={`w-10 h-10 text-plasma-violet transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={isHovered ? 2 : 1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
      </svg>
    ),
    rigor: (
      <svg className={`w-10 h-10 text-logo-blue transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <Card className={`flex flex-col items-center text-center h-full transition-all duration-300 ${isHovered ? 'shadow-[0_0_20px_rgba(91,192,235,0.3)]' : ''}`}>
      <div className="mb-3">
        {iconMap[value.icon as keyof typeof iconMap]}
      </div>
      <h3 className={`text-xl font-bold mb-1 text-titanium-white transition-all duration-300 ${isHovered ? 'text-logo-blue' : ''}`}>
        {value.title}
      </h3>
